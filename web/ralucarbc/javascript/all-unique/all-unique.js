...
const hasUniqueChars = (str) =>
  str.length === [...new Set(str.split(""))].length ? true : false;
divOutput.textContent =
  `The output (true/false if a string does/does not contain only unique characters) is: ` +
  hasUniqueChars(string) +
  `.`;

divFunction.textContent =
  `The function that does this is const hasUniqueChars = ` + hasUniqueChars;
